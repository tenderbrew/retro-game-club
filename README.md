...
- `index.html` — hero banner + speech bubble, sidebar calendar, upcoming events, stat strip, top-players list, and the **RetroRGB news fallback** (refresh the static `#retro-news-feed` items to current RetroRGB posts — the client-side fetch has a baked-in fallback that otherwise goes stale)
- `game-of-the-month.html` — add the new month's card to the current year grid
- **`games/*.html` navigation — ALL pages, not just the new one** — rebuild every monthly game page's `game-nav`: complete the dropdown with the new game and repoint Next/Latest so "Latest" is the newest game (see the nav invariant in **Consistency rules**)
...
### Consistency rules (checked by `node tools/audit.js`)

Site-wide invariants — keep them true by construction. **Run `node tools/audit.js` from the repo root before committing a rollover** — it's a fast, dependency-free checker that fails on any broken invariant below (rarity tiers, profile Rarity Breakdowns, nav dropdowns + "Latest" links, member count, missing images, broken internal links, each profile's points/rank vs the trophy holder lists, and elapsed heatmap months still marked `heat-future`). If you *intentionally* change an invariant, update the script and this list in the same commit. (Not yet auto-checked: whether an *active* heatmap month shows the correct medal colour — verify that by hand.)

- **Member count / rarity denominator:** the club currently has **28 active members**. Every trophy rarity % is `round(holders ÷ 28 × 100)`, and every "N of NN members" / "out of NN club members" string uses **28** — on trophy pages, `trophy-challenges.html`, AND each profile's crown-jewel + "Rarest Trophy" spotlight cards. When membership changes, **recompute every rarity figure** and update this number here (and the `DENOM` constant in `tools/audit.js`).
- **Rarity tiers:** a trophy's rarity *tier* is a pure function of its holder count — **Legendary** ≤10% · **Rare** 11–20% · **Uncommon** 21–40% · **Common** >40% (0 holders = **Unclaimed**). On every trophy page the tier label, the `tcase-rarity-<tier>` CSS class, and the `N of 28 members (X%)` stat must all agree; and each profile's **Rarity Breakdown** buckets that member's trophies by these same tiers. Use **"Legendary"** everywhere — never the legacy "Ultra Rare". (This scheme is enforced by `tools/audit.js`; change the thresholds there + here together.)
...
- **Activity heatmaps tick forward:** every rollover, the just-elapsed month becomes an activity class (if the member earned a trophy) or **`heat-empty`** — titled, e.g. `title="May 2026"` — if they didn't. It is **never `heat-future`**; only genuinely-future months stay `heat-future`. Refresh the other silently-drifting per-profile spotlights the same pass: **Gold Rate**, the **current-streak badge** (a gap resets it to 1 — distinct from the "Longest Streak" historical best), and any **"Latest Trophy"** link (must point at the member's most recent award — including on `leaderboards.html`).
- **Game-page navigation is site-wide:** every monthly game page carries the full `game-nav` — a dropdown listing **all** monthly games, plus First / Previous / Next / **Latest** links, where "Latest" points at the newest game. Rebuild these on **all** pages every rollover, not just the new one — otherwise old pages keep a stale "Latest" and short dropdowns (exactly the drift the July 2026 audit found). Bonus pages keep their simpler "Back to Home / Past Games" nav.
